...
#[derive(Default)]
pub struct NotificationStreamState(pub Mutex<Option<AbortHandle>>);
...
async fn stream_once(app: &AppHandle) -> Result<(), String> {
    ...
    let query = "subscription { notificationAdded { id title subject description importance link timestamp formattedTimestamp } }";
    write
        .send(Message::Text(
            json!({ "id": "notifications", "type": "subscribe", "payload": { "query": query } })
                .to_string(),
        ))
        .await
        ...
        match value.get("type").and_then(|t| t.as_str()) {
            Some("next") => {
                if let Some(data) = value
                    ...
                    .and_then(|d| d.get("notificationAdded"))
                {
                    let _ = app.emit("unraid-notification", data.clone());
                }
            }
            Some("error") => {
                let payload = value
                ...
                let _ = app.emit(
                    "unraid-notification-error",
                    format!("Subscription rejected: {payload}"),
                );
            }
            Some("complete") => break,
            _ => {}
...
async fn run(app: AppHandle) {
    loop {
        if let Err(e) = stream_once(&app).await {
            let _ = app.emit("unraid-notification-error", e);
        }
...
pub async fn start_notification_stream(
    app: AppHandle,
    state: State<'_, NotificationStreamState>,
) -> Result<(), String> {
...
#[tauri::command]
pub fn stop_notification_stream(state: State<NotificationStreamState>) -> Result<(), String> {
    let mut guard = state.0.lock().map_err(|e| e.to_string())?;
